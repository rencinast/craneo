...
app.put('/update-profile', (req, res) => {
    ...
    if (extNumber && (extNumber.length < 1 || extNumber.length > 10)) {
        return res.status(400).json({ success: false, message: "El número exterior debe tener entre 1 y 10 dígitos." });
    }

    // Verificar que el número exterior sea un número válido
    if (extNumber && !/^\d+$/.test(extNumber)) {
        return res.status(400).json({ success: false, message: "El número exterior debe ser un número válido." });
    }
