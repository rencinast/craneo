document.addEventListener("DOMContentLoaded", function () {
  ...
  profileForm.addEventListener("submit", function (event) {
    ...
    if (extNumber.length < 1 || extNumber.length > 10) {
      showNotification(
        "El número exterior debe tener entre 1 y 10 dígitos.",
        "error"
      );
    ...
    if (!/^\d+$/.test(extNumber)) {
      showNotification(
        "El número exterior debe ser un número válido.",
        "error"
      );
      return; // Detener el envío del formulario
    }
    ...
    if (zip.length !== 5 || !/^\d{5}$/.test(zip)) {
      showNotification(
        "El código postal debe ser exactamente de 5 dígitos.",
        "error"
      );
      return; // Detener el envío del formulario
    }
    ...
    fetch("/api/profiles/update-profile", {
      method: "PUT",
      headers: {
      ...
      .then((data) => {
        if (data.success) {
          ...
          showNotification(
            "Error al actualizar el perfil: " + data.message,
            "error"
          );
        }
      })
  ...
  function loadUserInfo(username) {
    fetch(`/api/profiles/get-user-info?username=${username}`)
      ...
      fetch(`/api/users/${userId}`, {
        ...
        .then((data) => {
          if (data.success) {
            ...
            return fetch("api/profiles/logout", {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
              },
            ...
            showNotification(
              "Error al eliminar la cuenta: " + data.message,
              "error"
            );
          }
        })
        ...
        .then((logoutData) => {
          if (logoutData.status === "success") {
            showNotification(
              "Cuenta eliminada y sesión cerrada con éxito!",
              "success"
            );
        ...
        .catch((error) => {
          console.error("Error:", error);
          showNotification(
            "Error al eliminar la cuenta o cerrar sesión.",
            "error"
          );
        });
    }
